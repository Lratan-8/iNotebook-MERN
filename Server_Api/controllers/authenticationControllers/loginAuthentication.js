...
const login = async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            success = false
            return res.status(400).json({ error: "Please try to login with correct credentials", success }); //if the user with that email doesn't exist in the database
        }
